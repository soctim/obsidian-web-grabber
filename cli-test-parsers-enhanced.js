...
const TypeScriptParserLoader = require('./ts-parser-loader');

class ParserTester {
    constructor() {
        this.parsers = new Map();
        this.results = [];
        this.tsLoader = new TypeScriptParserLoader();
    }

    // Load all parsers from the parsers directory (both .js and .ts)
    async loadParsers() {
        ...
        const jsFiles = fs.readdirSync(parsersDir).filter(file => file.endsWith('.js'));
        const tsFiles = fs.readdirSync(parsersDir).filter(file => file.endsWith('.ts'));
        
        console.log(`📁 Found ${jsFiles.length} JavaScript parser files and ${tsFiles.length} TypeScript parser files in ${parsersDir}`);
        
        // Load JavaScript parsers
        for (const file of jsFiles) {
            try {
                ...
                if (parserModule && parserModule.parser) {
                    ...
                    console.log(`✅ Loaded JS parser: ${parserModule.parser.name} (${parserModule.parser.id})`);
                } else {
                    console.warn(`⚠️  Could not load JS parser from ${file}`);
                }
            } catch (error) {
                console.error(`❌ Error loading JS parser from ${file}:`, error.message);
            }
        }
        
        // Load TypeScript parsers using the TypeScript loader
        for (const file of tsFiles) {
            try {
                const filePath = path.join(parsersDir, file);
                const parserModule = this.tsLoader.loadParser(filePath);
                
                if (parserModule && parserModule.parser) {
                    this.parsers.set(parserModule.parser.id, parserModule.parser);
                    console.log(`✅ Loaded TS parser: ${parserModule.parser.name} (${parserModule.parser.id})`);
                } else {
                    console.warn(`⚠️  Could not load TS parser from ${file}`);
                }
            } catch (error) {
                console.error(`❌ Error loading TS parser from ${file}:`, error.message);
            }
        }
    ...
    printHelp() {
        console.log(`
🔧 Enhanced Parser Testing CLI Tool (Supports JS & TypeScript)

USAGE:
  node cli-test-parsers-enhanced.js <command> [options]
...
EXAMPLES:
  node cli-test-parsers-enhanced.js test-url fanficsme "https://fanfics.me/fic37182"
  node cli-test-parsers-enhanced.js test-file reddit "./test-page.html"
  node cli-test-parsers-enhanced.js test-all "https://reddit.com/r/programming"
  node cli-test-parsers-enhanced.js validate fanficsme
  node cli-test-parsers-enhanced.js list
...
PARSER SUPPORT:
  ✅ JavaScript (.js) - Loaded directly
  ✅ TypeScript (.ts) - Compiled at runtime
        `);
    }
}
    ...
    try {
        ...
        switch (command) {
            ...
            default:
                ...
                console.log('Run "node cli-test-parsers-enhanced.js help" for usage information');
                process.exit(1);
        }
